...
export default async function BookingConfirmedPage({
  ...
  const booking = await prisma.booking.findUnique({
    ...
    include: { lead: { select: { firstName: true, source: true } }, serviceOrder: { select: { bookingToken: true } } },
  });
  if (!booking) notFound();

  return (
    <main className="mx-auto flex w-full max-w-xl flex-1 flex-col justify-center gap-5 px-5 py-16">
      <p className="text-sm font-semibold tracking-wide text-[var(--color-accent)] uppercase">{booking.kind === "consulting" ? "Séance de conseil réservée" : "C'est réservé"}</p>
